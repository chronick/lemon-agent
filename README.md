...
orchestration · `06` small tools · `07` JIT HTML views · `08`
protocol → hooks

Each chapter also names its traditional-engineering ancestor ("you've
met this before"): TDD/Red–Green, postmortem action items, RFCs, code
review + red teams, map-reduce, the Unix philosophy, data/presentation
separation, pre-commit hooks.
...
src/          Astro site (static) — LEMON AUDIO visual language, light rendition
...
CSS-only (radios) today, so examples need no runtime — but static-first
is a preference, not a vow: if an example genuinely needs a script,
add the script.
...
database. Raw markdown is served at `/guide/*.md`
