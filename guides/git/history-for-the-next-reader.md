...
`git blame` archaeology are ancient craft. The agent twist doubles the
audience and the authorship: messages are now written *by* agents and
read *by* agents, so their quality directly sets how much context a
